...
4. 首次全称后使用标准简称（如证监会/保监会/银监会）是允许的，不应单独判错。
5. 已正确写明《证券法》“1998年通过、1999年施行”或 Shibor“最初16家 / 现行18家 + 先看题干年份口径”的，不应因句式偏好继续打回。
6. 像“核心识别点为”“主要变化包括”“时间线如下”这类紧凑笔记体连接语是允许的，不要误判为客服腔或总结腔。
7. 只有会影响考试理解、事实正确性、关键知识覆盖或课程笔记交付形态的真实问题，才可以判为 REVISE。
8. 如果只剩轻微措辞、风格偏好、表格/列表选择、可继续优化但不影响交付的建议，应该判 PASS，并把这些内容放到“可优化项”里。
